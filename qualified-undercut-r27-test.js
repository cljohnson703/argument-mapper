...
try{const result=dom.window.eval(`(()=>{
 ...
 premise.children[0].texts=['If E, then it has not been shown that Q','E'];premise.children[0].children=[Object.assign(n('WW','weak-objection',['If F, then it has not been shown that E','F']),{targetIndex:1})];r=read([main]);
 check(r.v('M',0).status!=='established','weakly challenged undercut does not warrant exclusion claim');premise.children[0].children=[];
 const weak=n('RW','weak-objection',['If E, then it has not been shown that R','E']);
 ...
 premise.children=[Object.assign(n('W','weak-rebuttal',['If E, then it has not been shown that Q','E']),{targetIndex:1})];
