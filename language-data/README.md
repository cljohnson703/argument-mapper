...
The September 20 follow-up adds eight further source-verified past-alternative families, for fourteen reviewed families and 269 generated past mappings in total. The source snapshots are unchanged. Conditional-inference tests exercise the reviewed alternates in both orders, and universal-inference tests exercise all 281 imported plural mappings. Homographs and sense-sensitive alternatives remain excluded from automatic expansion; the 419-entry noun snapshot is not an exhaustive list of every possible English plural.

Four reviewed noun variants bring the generated plural mappings to 285: `octopi`, `dwarves`, `zeros` and `zeroes`. `reviewed-variants.json` records a dictionary source for each. These are explicit additions; no general Latin-ending conversion is applied. Tests also preserve capital-marked proper names and reject existential import from a universal statement alone.
